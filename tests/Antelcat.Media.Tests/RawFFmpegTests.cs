...
internal unsafe class RawFFmpegTests {
	[Test]
	public void SwrTest() {
		long sourceChannelLayout = ffmpeg.av_get_default_channel_layout(1);
		long destinationChannelLayout = ffmpeg.av_get_default_channel_layout(2);
		...
		var filename = Path.GetFullPath(@".\SwrTest.wav");
